...
typedef struct __mavlink_lcm_transport_t
{
 ...
 char payload[124]; ///< Payload data that is all or part of an LCM message.
} mavlink_lcm_transport_t;

#define MAVLINK_MSG_ID_LCM_TRANSPORT_LEN 146
#define MAVLINK_MSG_ID_223_LEN 146

#define MAVLINK_MSG_LCM_TRANSPORT_FIELD_PAYLOAD_LEN 124
...
#define MAVLINK_MESSAGE_INFO_LCM_TRANSPORT { \
	...
	{  { "utime", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_lcm_transport_t, utime) }, \
         ...
         { "payload", NULL, MAVLINK_TYPE_CHAR, 124, 22, offsetof(mavlink_lcm_transport_t, payload) }, \
         } \
}
...
static inline uint16_t mavlink_msg_lcm_transport_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint64_t utime, uint16_t msg_id, uint32_t message_part_counter, uint32_t message_part_total, uint32_t payload_size, const char *payload)
{
	...
	_mav_put_char_array(buf, 22, payload, 124);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, 146);
#else
	...
	mav_array_memcpy(packet.payload, payload, sizeof(char)*124);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, 146);
#endif

	msg->msgid = MAVLINK_MSG_ID_LCM_TRANSPORT;
	return mavlink_finalize_message(msg, system_id, component_id, 146, 52);
}
						           ...
						           uint64_t utime,uint16_t msg_id,uint32_t message_part_counter,uint32_t message_part_total,uint32_t payload_size,const char *payload)
{
	...
	msg->msgid = MAVLINK_MSG_ID_LCM_TRANSPORT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 146, 52);
}
...
{
	return mavlink_msg_lcm_transport_pack(system_id, component_id, msg, lcm_transport->utime, lcm_transport->msg_id, lcm_transport->message_part_counter, lcm_transport->message_part_total, lcm_transport->payload_size, lcm_transport->payload);
}
...
static inline void mavlink_msg_lcm_transport_send(mavlink_channel_t chan, uint64_t utime, uint16_t msg_id, uint32_t message_part_counter, uint32_t message_part_total, uint32_t payload_size, const char *payload)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[146];
	_mav_put_uint64_t(buf, 0, utime);
	...
	_mav_put_uint16_t(buf, 20, msg_id);
	_mav_put_char_array(buf, 22, payload, 124);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LCM_TRANSPORT, buf, 146, 52);
#else
	mavlink_lcm_transport_t packet;
	packet.utime = utime;
	packet.message_part_counter = message_part_counter;
	...
	packet.msg_id = msg_id;
	mav_array_memcpy(packet.payload, payload, sizeof(char)*124);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LCM_TRANSPORT, (const char *)&packet, 146, 52);
#endif
}
...
{
	return _MAV_RETURN_char_array(msg, payload, 124,  22);
}
...
#else
	memcpy(lcm_transport, _MAV_PAYLOAD(msg), 146);
#endif
}
